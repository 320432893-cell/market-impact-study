...
FIXED_QUALITY_CODE_DIRS = {"market-impact-study"}
...
COMMANDS: dict[str, str] = {
    "python-compile": "uv run python -m compileall -q market-impact-study tests tools",
    ...
    "semgrep": "uv run semgrep --disable-version-check --metrics=off --config .semgrep --no-git-ignore market-impact-study tests tools .ai-config",
    ...
    "radon-cc": "uv run radon cc market-impact-study -s -n C",
    "radon-mi": "uv run radon mi market-impact-study -s",
    "vulture": "uv run vulture market-impact-study --min-confidence 80",
    "deptry": "uv run deptry .",
    "reachability": "python3 tools/reachability_probe.py market-impact-study",
    "name-health": "python3 tools/name_health.py market-impact-study",
    "import-cycles": "python3 tools/import_cycles.py market-impact-study",
...
PROFILES: dict[str, list[str]] = {
    "quick": ["coverage-audit", "python-compile", "rule-tool-contracts", "ruff-staged", "semgrep"],
    "completion": ["rule-tool-contracts"],
    "manual": ["ruff-check", "ruff-format-check", "pip-audit"],
    "deep": [
        "radon-cc",
        "radon-mi",
        "vulture",
...
def run_changed() -> int:
    ...
    if not code_paths and not contract_changed:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        nargs="?",
        default="quick",
        choices=[*PROFILES, *COMMANDS, "detect-secrets", "list", "new"],
    )
