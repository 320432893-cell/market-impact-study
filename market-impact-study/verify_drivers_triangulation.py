...
# ---------- 小样本严谨层:CR2+BM自由度 / 有效簇数 / Romano-Wolf / 贝叶斯多层 ----------
def _inv_sqrt(M: np.ndarray) -> np.ndarray:
    w, V = np.linalg.eigh(M)
    w = np.clip(w, 1e-10, None)
    return V @ np.diag(1 / np.sqrt(w)) @ V.T


def ols_cr2(x: np.ndarray, y: np.ndarray, g: np.ndarray, k: int) -> tuple[float, float, float]:
    # CR2(去偏聚类稳健 SE)+ Bell-McCaffrey Satterthwaite 自由度(少簇正确小样本 t 检验)
    ...
    for gg in gids:
        m = g == gg
        Xg, ug = x[m], u[m]
        A = _inv_sqrt(np.eye(int(m.sum())) - Xg @ bread @ Xg.T)
        ut = A @ ug
        sc = Xg.T @ ut
        meat += np.outer(sc, sc)
        bg = A @ (Xg @ (bread @ s))
        ...
        cols.append(col)
    V = bread @ meat @ bread
    se = float(np.sqrt(max(V[k, k], 1e-12)))
    D = np.column_stack(cols)
    M2 = D - x @ (bread @ (x.T @ D))  # (I-H)D,不显式建 H
    num = float((M2 ** 2).sum())
    den = float(np.sum((M2.T @ M2) ** 2))
    dof = num ** 2 / den if den > 0 else len(gids) - 1
    return float(beta[k]), se, float(dof)


def effective_clusters(x: np.ndarray, g: np.ndarray, k: int) -> float:
    # Carter-Schnepel-Steigerwald 风格有效簇数:关键回归元残差(FWL)的组内平方和不均衡度
    others = [j for j in range(x.shape[1]) if j != k]
    Xo, xk = x[:, others], x[:, k]
    xt = xk - Xo @ np.linalg.lstsq(Xo, xk, rcond=None)[0]
    gamma = np.array([np.sum(xt[g == gg] ** 2) for gg in np.unique(g)])
    return float(gamma.sum() ** 2 / np.sum(gamma ** 2))


def cr2_pack(x: np.ndarray, y: np.ndarray, g: np.ndarray, k: int) -> dict:
    from scipy import stats

    b, se, dof = ols_cr2(x, y, g, k)
    p = float(2 * stats.t.sf(abs(b / se), dof))
    return {"se_cr2": round(se, 3), "dof_bm": round(dof, 1), "p_cr2": round(p, 3),
            "effective_G": round(effective_clusters(x, g, k), 1)}


def romano_wolf(dzs: dict) -> dict:
    # 跨假设族错误率:共享簇权重的野聚类自助 + stepdown(防"多测几次撞上")
    keys = list(dzs)
    obs_t, fitr, resr = {}, {}, {}
    for h in keys:
        d = dzs[h]
        beta, se, _ = ols_cluster(d["x"], d["y"], d["g"])
        obs_t[h] = beta[d["k"]] / se[d["k"]]
        others = [j for j in range(d["x"].shape[1]) if j != d["k"]]
        br, _, ur = ols_cluster(d["x"][:, others], d["y"], d["g"])
        fitr[h], resr[h] = d["x"][:, others] @ br, ur
    firms = sorted(set().union(*[set(dzs[h]["g"]) for h in keys]))
    rng = np.random.RandomState(RNG)
    T = np.zeros((B, len(keys)))
    for b in range(B):
        w = {f: (1.0 if rng.rand() < 0.5 else -1.0) for f in firms}
        for j, h in enumerate(keys):
            d = dzs[h]
            vv = np.array([w[f] for f in d["g"]])
            ystar = fitr[h] + vv * resr[h]
            bb, sb, _ = ols_cluster(d["x"], ystar, d["g"])
            T[b, j] = bb[d["k"]] / sb[d["k"]]
    order = sorted(range(len(keys)), key=lambda j: -abs(obs_t[keys[j]]))
    active, prev, out = list(order), 0.0, {}
    for idx in order:
        maxd = np.max(np.abs(T[:, active]), axis=1)
        p = max((np.sum(maxd >= abs(obs_t[keys[idx]])) + 1) / (B + 1), prev)
        out[keys[idx]] = round(float(p), 3)
        prev = p
        active = [a for a in active if a != idx]
    return out


def bayes_multilevel(d: dict) -> dict:
    # 贝叶斯多层(公司+年度随机截距,弱信息先验防方差塌零);报后验区间+P(β<0)+收敛
    import jax
    ...
    xk, y = jnp.asarray(d["xkey"]), jnp.asarray(d["y"])
    firm, yr = jnp.asarray(d["firm"]), jnp.asarray(d["yr"])
    nf, ny = int(d["firm"].max() + 1), int(d["yr"].max() + 1)

    def model(xk, firm, yr, y=None):
        a = numpyro.sample("a", dist.Normal(0, 1.0))
        beta = numpyro.sample("beta", dist.Normal(0, 1.0))
        sf = numpyro.sample("sigma_firm", dist.HalfNormal(1.0))
        sy = numpyro.sample("sigma_year", dist.HalfNormal(1.0))
        sig = numpyro.sample("sigma", dist.HalfNormal(1.0))
        uf = numpyro.sample("uf", dist.Normal(0, 1).expand([nf]))
        uy = numpyro.sample("uy", dist.Normal(0, 1).expand([ny]))
        mu = a + beta * xk + sf * uf[firm] + sy * uy[yr]
        numpyro.sample("obs", dist.Normal(mu, sig), obs=y)

    mcmc = MCMC(NUTS(model), num_warmup=1000, num_samples=1000, num_chains=2, progress_bar=False)
    mcmc.run(jax.random.PRNGKey(0), xk, firm, yr, y=y)
    bs = np.asarray(mcmc.get_samples()["beta"])
    bc = np.asarray(mcmc.get_samples(group_by_chain=True)["beta"])
    return {"mean": round(float(bs.mean()), 3),
            "ci95": [round(float(np.percentile(bs, 2.5)), 3), round(float(np.percentile(bs, 97.5)), 3)],
            "P_neg": round(float((bs < 0).mean()), 3),
            "rhat": round(float(diag.gelman_rubin(bc)), 3)}


def _codes(a: np.ndarray) -> np.ndarray:
    return pd.Series(a).astype("category").cat.codes.to_numpy()
...
def run_H1(cp: pd.DataFrame) -> dict:  # noqa: N802 - 假设编号
    ...
    return {
        ...
        **cr2_pack(twoway, y, g, 1),
        "passes": passes,
        "verdict": consilience(passes),
        "_dz": {"x": twoway, "y": y, "g": g, "k": 1, "xkey": xr,
                "firm": _codes(g), "yr": _codes(cp["yr"].to_numpy())},
    }
...
def _run_level(df: pd.DataFrame, feat: np.ndarray, name: str, *, do_leadlag: str | None = None) -> dict:
    ...
    return {
        ...
        **cr2_pack(base, y, g, 1),
        "passes": passes,
        "verdict": consilience(passes),
        "_dz": {"x": base, "y": y, "g": g, "k": 1, "xkey": xz,
                "firm": _codes(g), "yr": _codes(df["year"].to_numpy())},
    }
...
def main() -> None:
    ...
    # 小样本严谨层:跨假设多重比较(Romano-Wolf) + 贝叶斯多层(独立第二范式)
    dzs = {h: results[h].pop("_dz") for h in results}
    rw = romano_wolf(dzs)
    for h in results:
        results[h]["p_romano_wolf"] = rw[h]
        results[h]["bayes"] = bayes_multilevel(dzs[h])
