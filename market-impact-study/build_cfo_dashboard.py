...
FUND_PANEL = C.parent / "fundamental_panel.csv"
...
YIWEI = "300590.SZ"
NAMES = {"002313": "日海智能", "002869": "金溢科技", "002881": "美格智能", "002970": "锐明技术",
...
def bundle() -> dict:
    ...
    # 面板覆盖(非平衡):从基本面面板算每家报告期起止年与期数(报告 F2 的原生重绘数据)
    code2name = {p["code"]: p["firm"] for p in mc["per_firm"]}
    cov: dict[str, dict] = {}
    with FUND_PANEL.open(encoding="utf-8", newline="") as fh:
        for row in csv.DictReader(fh):
            code, yr = row["ts_code"], int(row["end_date"][:4])
            r = cov.setdefault(code, {"start": yr, "end": yr, "n": 0})
            r["start"], r["end"], r["n"] = min(r["start"], yr), max(r["end"], yr), r["n"] + 1
    coverage = sorted(
        ({"firm": code2name.get(c) or NAMES.get(c.split(".")[0], c), "start": v["start"], "end": v["end"], "n": v["n"], "is_yiwei": c == YIWEI}
    ...
    return {
        ...
        "tri": {
            k: {
                ...
                "sign": v.get("sign_consist"),  # 设定维:多设定符号一致率
                "lofo": v.get("lofo_consist"),  # 泛化维:LOFO 方向一致率
                "rw": v.get("p_romano_wolf"),  # 多重比较校正 p
                "G": v.get("effective_G"),  # 有效独立簇数
            }
        ...
        "model": {
            ...
            "l1path": json.loads((C / "l1_path.json").read_text(encoding="utf-8")),  # L1 正则化路径(⑥ 特征选择页)
            "sizebins": json.loads((C / "size_bins.json").read_text(encoding="utf-8")),  # 规模6档估值(④ 标签设计页)
            "stab": json.loads((C / "stability_bootstrap.json").read_text(encoding="utf-8")),  # SHAP 自助稳定性(⑬ 可解释性页)
            "wbdir": json.loads((C / "wb_direction.json").read_text(encoding="utf-8")),  # 白盒-黑盒方向对照(⑬ 可解释性页)
        },
    }
...
.body{flex:1;padding:16px var(--mr) 50px var(--ml);display:flex;flex-direction:column;gap:20px;overflow:hidden;justify-content:space-between;position:relative;z-index:1}
...
.hbar-plot{flex:1;display:flex;align-items:flex-end;justify-content:space-around;border-left:1.5px solid #d7d7d7;border-bottom:1.5px solid #d7d7d7;padding:0 14px;background:repeating-linear-gradient(to top,#ededed 0 1px,transparent 1px 25%),#fff}
.hbar-set{height:100%;display:flex;align-items:flex-end;gap:20px}
.hbar-bar{width:66px;border-radius:5px 5px 0 0;position:relative;display:flex;justify-content:center;align-items:flex-start;box-shadow:0 1px 2px rgba(0,0,0,.06)}
.hbar-bar .v{color:#fff;font-size:22px;font-weight:700;margin-top:10px}
.residhist{display:flex;flex-direction:column;flex:1;min-height:0}
.residhist .hbar-bar .v{color:#2c496e}
...
.hm-cell{display:flex;align-items:center;justify-content:center;border-radius:3px;font-size:16px;font-weight:600;line-height:1.05;overflow:hidden;text-align:center}
.hm-corner{background:transparent}
.hm-row{justify-content:flex-end;padding-right:8px;color:var(--ink);font-size:16px}
.hm-col{color:var(--ink);font-size:16px;writing-mode:vertical-rl;text-orientation:upright;letter-spacing:1px;white-space:nowrap}
...
table.t th{background:#efe8db;color:var(--ink);text-align:left;padding:15px 14px;font-weight:700;border-bottom:2px solid var(--brand)}
...
.cover-no{position:absolute;right:96px;bottom:60px;color:#e6ddcf;font-size:112px;line-height:1;font-weight:700;z-index:0}
/* ===== 目录 ===== */
...
.panel table.t.compact th{font-size:17.5px;padding:8px 12px}.panel table.t.compact td{padding:4px 12px;font-size:17.5px}
/* ===== 问题页:研究问题 hero + 干净目标列表 ===== */
.qhero{font-size:35px;line-height:1.5;font-weight:700;color:var(--brand-dark);margin:6px 0 34px}
.thesis{border-left:7px solid var(--brand);background:var(--soft);border-radius:0 10px 10px 0;padding:24px 32px;margin:6px 0 30px}
.thesis-tag{font-size:19px;font-weight:700;color:var(--brand);letter-spacing:2px;margin-bottom:12px}
.thesis-main{font-size:33px;line-height:1.5;font-weight:700;color:var(--brand-dark)}
.thesis-goals{display:flex;gap:14px;flex-wrap:wrap;margin-top:20px}
.gchip{border:1px solid var(--frame);background:#fff;border-radius:22px;padding:9px 20px;font-size:20px;font-weight:600;color:var(--dark)}
