...
# 驱动 + 单调约束方向。约束只施加于经济方向无歧义且经三角验证的杠杆(资产负债率 -1, H2 5/5);
# 盈利/成长/周转/费用等方向在本样本未获独立验证,一律不约束(0),由数据决定其方向与符号。
DRIVERS = {
    "f_net_margin": ("净利率", 0),
    "f_gross_margin": ("毛利率", 0),
    "f_op_margin": ("营业利润率", 0),
    "f_roe": ("ROE", 0),
    "f_roa": ("ROA", 0),
    "f_asset_turn": ("资产周转率", 0),
    "f_rev_yoy": ("营收同比", 0),
    "f_ni_yoy": ("净利同比", 0),
    "f_rev_cagr3": ("营收3年CAGR", 0),
    "f_ocf_to_rev": ("经营现金流/营收", 0),
    "f_fcf_to_rev": ("自由现金流/营收", 0),
    ...
    "f_ar_turn": ("应收周转", 0),
    "f_fa_turn": ("固定资产周转", 0),
    "f_saleexp_ratio": ("销售费用率", 0),
    "f_adminexp_ratio": ("管理费用率", 0),
    "f_finaexp_ratio": ("财务费用率", 0),
    "f_dt_ni_yoy": ("扣非净利增速", 0),
    "f_ocf_yoy": ("经营现金流增速", 0),
    "f_debt_to_eqt": ("产权比率", 0),
    "f_overseas_share": ("海外收入占比", 0),
    "d_net_margin": ("净利率趋势Δ", 0),
    "d_gross_margin": ("毛利率趋势Δ", 0),  # 趋势特征:再定价触发
...
SPACE = {
    ...
    # 有效样本仅 14 家公司,a priori 限制树复杂度防过拟合(原设计浅树档位 num_leaves=7/depth=3)
    "num_leaves": [4, 5, 6, 7],
    "max_depth": [2, 3],
...
def main() -> None:
    ...
    # 特征缩尾(2%/98%),与目标同口径:抑制个别公司的极端比率(如产权比率)对线性外推/选特征的破坏
    ...
    # SHAP 不可裸示:① 高缺失特征(中位填充→importance 多为常数假象)标记隔离;② 每特征挂 FE 真伪裁决(复核要求)
    fe = fe_validate(df, cols, y)
    fe_verdict = {r["feat"]: r["verdict"] for r in fe}
    coverage = {DRIVERS[c][0]: round(float(1 - df[c].isna().mean()), 3) for c in cols}
    HIGH_MISS = 0.30  # 覆盖率<70% 的特征,SHAP 归因不可靠,high_missing=True 供下游过滤
    imp = sorted(
        [
            {
                "feat": DRIVERS[c][0],
                "shap": round(float(mean_abs[j]), 4),
                "dir": DRIVERS[c][1],
                "coverage": coverage[DRIVERS[c][0]],
                "high_missing": bool(coverage[DRIVERS[c][0]] < 1 - HIGH_MISS),
                "fe_verdict": fe_verdict.get(DRIVERS[c][0], "未检验"),
            }
            for j, c in enumerate(cols)
        ],
        key=lambda z: -z["shap"],
