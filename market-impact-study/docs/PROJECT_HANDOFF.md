...
> ⚠️ **复现注意**:① 主线脚本用 `market-impact-study/...` 根相对路径,**必须从仓库根 `market-impact-study/`(独立仓) 运行**;② 改了产生 JSON 的脚本后**务必重跑产物再 build 仪表板**(曾出现源码修了、JSON 没重跑导致仪表板显示 undefined 的漂移);③ 尚无 `run_pipeline.py` 串全链,当前需手动按上序执行。
